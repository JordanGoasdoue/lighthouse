...
func makePipelineRun(ctx context.Context, lj v1alpha1.LighthouseJob, namespace string, logger *logrus.Entry, idGen buildIDGenerator, c client.Reader) (*pipelinev1beta1.PipelineRun, error) {
	// First validate.
	...
	for _, key := range sets.StringKeySet(env).List() {
		val := pipelinev1beta1.ParamValue{
...
func makePipelineRunV1(ctx context.Context, lj v1alpha1.LighthouseJob, namespace string, logger *logrus.Entry, idGen buildIDGenerator, c client.Reader) (*pipelinev1.PipelineRun, error) {
	// First validate.
	if lj.Spec.PipelineRunSpec == nil {
		return nil, errors.New("no PipelineSpec defined")
	}
	...
	prLabels, annotations := jobutil.LabelsAndAnnotationsForJob(lj, buildID)
	specCopy := lj.Spec.PipelineRunSpecV1.DeepCopy()
	generateName := jobutil.GenerateName(&lj.Spec)
	...
	if len(lj.Spec.PipelineRunParams) > 0 {
		...
		paramNames, err := determineGitCloneOrMergeTaskParamsV1(ctx, &p, c)
		if err != nil {
			return nil, err
		}
		if paramNames == nil {
...
func determineGitCloneOrMergeTaskParams(ctx context.Context, pr *pipelinev1beta1.PipelineRun, c client.Reader) (*gitTaskParamNames, error) {
	if pr == nil {
	...
	if pr.Spec.PipelineSpec != nil {
		...
		pipeline := pipelinev1beta1.Pipeline{ObjectMeta: metav1.ObjectMeta{Name: pr.Spec.PipelineRef.Name, Namespace: pr.Namespace}}
		...
		if task.TaskRef != nil {
			if task.TaskRef.Name == gitCloneCatalogTaskName {
				for _, p := range task.Params {
					if p.Name == gitCloneURLParam && p.Value.Type == pipelinev1beta1.ParamTypeString {
						paramNames.urlParam = extractPipelineParamFromTaskParamValue(p.Value.StringVal)
					}
					if p.Name == gitCloneRevisionParam && p.Value.Type == pipelinev1beta1.ParamTypeString {
			...
			if task.TaskRef.Name == gitMergeCatalogTaskName {
				for _, p := range task.Params {
					...
					if p.Name == gitMergeBatchRefsParam && p.Value.Type == pipelinev1beta1.ParamTypeString {
...
func determineGitCloneOrMergeTaskParamsV1(ctx context.Context, pr *pipelinev1.PipelineRun, c client.Reader) (*gitTaskParamNames, error) {
	if pr == nil {
		return nil, errors.New("provided PipelineRun is nil")
	}
