...
var (
	...
	defaultParameterSpecs = []pipelinev1beta1.ParamSpec{
		{
	...
	defaultParameterSpecsV1 = []pipelinev1.ParamSpec{
		{
			Description: "the unique build number",
			Name:        "BUILD_ID",
			Type:        "string",
	...
	defaultParametersV1 = ToParamsV1(defaultParameterSpecsV1)
)
...
func DefaultPipelineParameters(prs *pipelinev1beta1.PipelineRun) (*pipelinev1beta1.PipelineRun, error) {
	...
	for i := range ps.Tasks {
		...
		if task.TaskSpec != nil {
			...
			if task.TaskSpec.StepTemplate == nil {
				task.TaskSpec.StepTemplate = &pipelinev1beta1.StepTemplate{}
			}
...
func DefaultPipelineParametersV1(prs *pipelinev1.PipelineRun) (*pipelinev1.PipelineRun, error) {
	if prs.Annotations != nil && prs.Annotations[DefaultParameters] == "false" {
		return prs, nil
	}
	ps := prs.Spec.PipelineSpec
	...
	ps.Params = addDefaultParameterSpecsV1(ps.Params, defaultParameterSpecsV1)

	for i := range ps.Tasks {
		task := &ps.Tasks[i]
		task.Params = addDefaultParametersV1(task.Params, defaultParametersV1)
		if task.TaskSpec != nil {
			task.TaskSpec.Params = addDefaultParameterSpecsV1(task.TaskSpec.Params, defaultParameterSpecsV1)
...
func addDefaultParameterSpecs(params []pipelinev1beta1.ParamSpec, defaults []pipelinev1beta1.ParamSpec) []pipelinev1beta1.ParamSpec {
...
func addDefaultParameterSpecsV1(params []pipelinev1.ParamSpec, defaults []pipelinev1.ParamSpec) []pipelinev1.ParamSpec {
	for _, dp := range defaults {
		found := false
		for i := range params {
			param := &params[i]
...
func addDefaultParameters(params []pipelinev1beta1.Param, defaults []pipelinev1beta1.Param) []pipelinev1beta1.Param {
	for _, dp := range defaults {
		found := false
		for i := range params {
			p := &params[i]
...
func addDefaultParametersV1(params []pipelinev1.Param, defaults []pipelinev1.Param) []pipelinev1.Param {
	for _, dp := range defaults {
		found := false
		for i := range params {
			p := &params[i]
...
func addDefaultParameterEnvVars(env []corev1.EnvVar, defaults []pipelinev1beta1.Param) []corev1.EnvVar {
	for _, dp := range defaults {
		name := dp.Name
...
func addDefaultParameterEnvVarsV1(env []corev1.EnvVar, defaults []pipelinev1.Param) []corev1.EnvVar {
	for _, dp := range defaults {
		name := dp.Name
		upperName := strings.ToUpper(name)
		if upperName != name {
